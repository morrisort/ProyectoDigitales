...
#include "axiLite2reg/axiLite2reg.h"
#include "Qarrays.h"
#include "QandConst.h"
#include "pixelRadiationRegistersQ_regs.h"

/*
 * XPAR redefines
 */
#define DYNCLK_BASEADDR 0 //XPAR_AXI_DYNCLK_0_BASEADDR
#define PIXELRADIATION_BASEADDR 0x44A20000
...
void DemoInitialize()
{
	...
	// load Q1, Q2 and constant
	int lowerIndex;
		int upperIndex;
		for (int i = 0; i < 16; i++){
			lowerIndex = i * LVECTOR;
			upperIndex = lowerIndex + LVECTOR - 1;
			setT (PIXELRADIATION_BASEADDR, DATAQRED1_OFFSET, QRed1, lowerIndex, upperIndex); //RedQ1
			setT (PIXELRADIATION_BASEADDR, DATAQRED2_OFFSET, QRed2, lowerIndex, upperIndex); //RedQ2
			setT (PIXELRADIATION_BASEADDR, DATAQGREEN1_OFFSET, QGreen1, lowerIndex, upperIndex); //GreenQ1
			setT (PIXELRADIATION_BASEADDR, DATAQGREEN2_OFFSET, QGreen2, lowerIndex, upperIndex); //GreenQ2
			setT (PIXELRADIATION_BASEADDR, DATAQBLUE1_OFFSET, QBlue1, lowerIndex, upperIndex); //BlueQ1
			setT (PIXELRADIATION_BASEADDR, DATAQBLUE2_OFFSET, QBlue2, lowerIndex, upperIndex); //BlueQ2
			Xil_Out32(PIXELRADIATION_BASEADDR + COMMAND_OFFSET, 9); // Load circular buffer command
		}
		...
		Xil_Out32(PIXELRADIATION_BASEADDR + COMMAND_OFFSET, 15); // Finish loading command
...
void DemoRun()
{
	...
	while (userInput != 'q')
	{
		...
		case '8':
			...
			break;
		case '9':
			upper = Xil_In32(PIXELRADIATION_BASEADDR + UPPERRESULT_OFFSET);
			lower = Xil_In32(PIXELRADIATION_BASEADDR + LOWERRESULT_OFFSET);
			sleep(1);
			upper = Xil_In32(PIXELRADIATION_BASEADDR + UPPERRESULT_OFFSET);
			lower = Xil_In32(PIXELRADIATION_BASEADDR + LOWERRESULT_OFFSET);
			//xil_printf("upper: %x \n\r", upper);
			...
			unsigned int arr[2];
			double result;
			//memcpy(&arr[0],&lower,4);
			//memcpy(&arr[1],&upper,4);
			memcpy((char*)&result, &lower, sizeof(lower));
			memcpy((char*)&result + sizeof(lower), &upper, sizeof(upper));
			//long long result;
			//memcpy(&result,arr,8);
			//double *floatResult; // floating point radiation result
			//floatResult = &result;
			//printf("the final result, in floating point notation is: %f \n\r", *floatResult);
			//printf("%f\n", result);
...
void DemoPrintMenu()
{
	unsigned int upper = 0;
	unsigned int lower = 0;
	// Reads pixel radiation 2 times to ensure getting a current number
	upper = Xil_In32(PIXELRADIATION_BASEADDR + UPPERRESULT_OFFSET);
	lower = Xil_In32(PIXELRADIATION_BASEADDR + LOWERRESULT_OFFSET);
	sleep(1);
	upper = Xil_In32(PIXELRADIATION_BASEADDR + UPPERRESULT_OFFSET);
	...
	xil_printf("**************************************************\n\r");
	xil_printf("*              Pixel Radiation Demo              *\n\r");
	xil_printf("**************************************************\n\r");
	...
	printf("*Current measured radiation:       %6.7f[W]*\n\r", result);
	xil_printf("**************************************************\n\r");
	xil_printf("\n\r");
	...
	xil_printf("7 - Get new radiation\n\r");
